# Recursos sobre la clase 2

## Sobre Formularios

* [An imperative guide to forms in Vue.js](https://blog.logrocket.com/an-imperative-guide-to-forms-in-vue-js-7536bfa374e0)
* [How to build a complete form with Vue.js](https://auralinna.blog/post/2018/how-to-build-a-complete-form-with-vue-js)
* [Vue.js — Forms, components and considerations](https://blog.webf.zone/vue-js-forms-components-and-considerations-d81b3ffe9efb)
* [Simple Vue.js Form Validation with Vuelidate](https://vuejsdevelopers.com/2018/08/27/vue-js-form-handling-vuelidate/)

## Sobre Comunicación entre componentes

* [VUE.JS COMPONENT PROPS](https://flaviocopes.com/vue-props/)
* [COMPONENT COMMUNICATION (PROPS / EVENTS) | VueJS 2](https://www.youtube.com/watch?v=PPmg7ntQjzc)
* [Intro to Vue.js: Components, Props, and Slots](https://css-tricks.com/intro-to-vue-2-components-props-slots/)
* [You will be surprised by the custom event in Vue js](https://medium.com/introcept-hub/you-will-be-surprised-custom-event-in-vue-js-c713afdff8e5)

## Sobre Propiedades computadas y Watchers

* [Introduction to Computed Properties in Vue.js](https://alligator.io/vuejs/computed-properties/)
* [Methods, Computed, and Watchers in Vue.js](https://css-tricks.com/methods-computed-and-watchers-in-vue-js/)
* [VueJS - Watch Property](https://www.tutorialspoint.com/vuejs/vuejs_watch_property.htm)
* [Filtros en VUE.js usando Computed Properties](https://rimorsoft.com/filtros-en-vue-js-usando-computed-properties)

## Sobre Conceptos avanzados de Vue

* [Understanding Component Slots with Vue.js](https://alligator.io/vuejs/component-slots/)
* [Using Filters in Vue.js](https://css-tricks.com/using-filters-vue-js/)
* [A simple, real-world Vue.js directive](https://codewithhugo.com/a-simple-real-world-vue.js-directive/)
* [Create an App With Vue.js Using Mixins Tutorial](https://www.youtube.com/watch?v=D3MGf1d2bUo)